...
export default function LoginForm({
  ...
  return (
    <div
      className="
        relative mx-auto w-[520px] max-w-[92vw]
    ...
    >
      <div className="rounded-3xl border-[4px] border-[#0A1635] bg-[#0C1D41] p-6">
        ...
        {/* === Misma funcionalidad: label + input === */}
          ...
          <div className="mt-2 rounded-2xl border-[4px] border-sky-700/70 bg-[#0E254F] px-4 py-2 shadow-[inset_0_2px_0_rgba(255,255,255,0.05)]">
            <input
              ...
              className="
                ...
                text-sky-300 placeholder:text-sky-400 caret-sky-300
              "
            />
          </div>
        </label>

        {/* Botón (mismo handler y disabled) */}
        <button
          ...
          className="
            ...
            transition-transform active:translate-y-[3px] disabled:opacity-60 disabled:cursor-not-allowed
          "
        >
        ...
        {!!loggedLabel && (
          <div className="mt-3 text-center text-sm font-bold text-sky-300/80">
            {loggedLabel}
          </div>
        )}
      </div>

      {/* Base decorativa inferior (opcional, solo estilo) */}
